...
public class Test1 {
    public static void main(String[] args) {
        String string = "20世纪90年代，硬件领域出现了单片式计算机系统，这种价格低廉的系统一出现就立即引起了自动控制领域人员的注意，因为使用它可以大幅度提升消费类电子产品（如电视机顶盒、面包烤箱、移动电话等）的智能化程度。Sun公司为了抢占市场先机，在1991年成立了一个称为Green的项目小组，帕特里克、詹姆斯·高斯林、麦克·舍林丹和其他几个工程师一起组成的工作小组在加利福尼亚州门洛帕克市沙丘路的一个小工作室里面研究开发新技术，专攻计算机在家电产品上的嵌入式应用。\n" +
        ...
        //  1.传统方式进行遍历,代码量大且不易维护效率低
        //  2.正则启动

        //  正则写法:
        //  1.创建一个匹配模式 Pattern对象,模式对象,可以理解成就是一个正则表达式
//        Pattern pattern = Pattern.compile("[a-zA-Z1-9]+");
//        Pattern pattern = Pattern.compile("[1-9]+");
        Pattern pattern = Pattern.compile("\\d+年");
        //  2.创建一个匹配对象,
        //  就是matcher匹配按照pattern(模式/样式) 到content中去匹配
        Matcher matcher = pattern.matcher(string);

        //  3.循环找符合匹配的内容
        //  matcher.find()找到则返回true 否则返回false
        while (matcher.find()) {
            //  匹配内容,文本到m.group(0)
